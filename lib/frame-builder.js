...
frame_builder[C.FRAME_TYPE.TX_REQUEST_64] = function(buffer, frame) {
    ...
    buffer.write(frame.destination64, length, 'hex');
    length += 8;

    buffer.writeUInt8(frame.options || 0x00, length++);
    if (typeof frame.data == "string") {
        buffer.write(frame.data, length, 'ascii');
        ...
        for (var i = 0; i < frame.data.length; i++) {
            ...
            if (frame.data[i] < 0x10) {
                console.log("int8")
                buffer.writeUInt8(frame.data[i], length++);
            } else if (frame.data[i]> 0x0f) {
                console.log("HEX")
                buffer.write(frame.data[i].toString(16), length++, 'HEX');
            }
            /*else {
             console.log("mini HEX", frame.data[i])
             buffer.write(frame.data[i], length++, 'HEX');
             }*/
        }
    }
    return length;
